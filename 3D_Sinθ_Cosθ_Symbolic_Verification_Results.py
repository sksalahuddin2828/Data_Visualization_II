...
# Create an animated plot of sine and cosine functions
fig_anim, ax_anim = plt.subplots()
x_anim = np.linspace(0, 360, 100)
line_sin, = ax_anim.plot(x_anim, np.sin(np.deg2rad(x_anim)), label='sin(theta)')
line_cos, = ax_anim.plot(x_anim, np.cos(np.deg2rad(x_anim)), label='cos(theta)')

def update(frame):
    line_sin.set_ydata(np.sin(np.deg2rad(x_anim + frame)))
    line_cos.set_ydata(np.cos(np.deg2rad(x_anim + frame)))
    return line_sin, line_cos

ani = FuncAnimation(fig_anim, update, frames=np.linspace(0, 360, 360), blit=True, interval=50)
plt.legend()
plt.show()
